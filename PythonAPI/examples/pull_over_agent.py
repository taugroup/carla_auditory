...
from agents.tools.misc import get_speed, is_within_distance, get_trafficlight_trigger_location,draw_waypoints


class PullOverAgent(object):
    """
    ...
    def set_destination(self, end_location, start_location=None,siren = False):
        """
        ...
        self.past_location = end_location
        start_waypoint = self._map.get_waypoint(start_location)
        end_waypoint = self._map.get_waypoint(end_location)

        route_trace = self.trace_route(start_waypoint, end_waypoint)
        if siren == True:
            self._local_planner.set_global_plan(route_trace,stop_waypoint_creation = True, clean_queue=True,siren = siren)
            
        else:
            self._local_planner.set_global_plan(route_trace, clean_queue=clean_queue)

    def set_destination_by_waypoint(self,end_waypoint,start_location=None):
        """
        This method creates a list of waypoints between a starting and ending location,
        based on the route returned by the global router, and adds it to the local planner.
        If no starting location is passed, the vehicle local planner's target location is chosen,
    ...
    def get_pull_over_location(self):
        vehicle_location = self._vehicle.get_location()
        vehicle_waypoint = self._map.get_waypoint(vehicle_location).next(30)
            
            #vehicle_waypoint= self._local_planner.get_incoming_waypoint_and_direction()
            #print(vehicle_waypoint)
            #self._local_planner.reset_vehicle()

        right_lane = vehicle_waypoint[0].get_right_lane()
        return right_lane
    ...
    def pull_over(self):
        vehicle_location = self._vehicle.get_location()
        vehicle_waypoint = self._map.get_waypoint(vehicle_location).next(10) # why next 10?
        
        if(type(vehicle_waypoint) is list):
            vehicle_waypoint = vehicle_waypoint[0]
        
        if(not vehicle_waypoint.get_right_lane()):
            return
        
        while(vehicle_waypoint.get_right_lane()):
            vehicle_waypoint = vehicle_waypoint.get_right_lane()

        dest = vehicle_waypoint.transform.location
        self.location = dest
